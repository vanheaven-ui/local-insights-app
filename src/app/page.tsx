...
export default function Home() {
  // State for the feedback text input by the user
  const [feedbackText, setFeedbackText] = useState<string>("");
  // State to store the analysis result received from the API
  ...
  // State for the dynamic header: true when scrolled, false when at top
  const [isHeaderScrolled, setIsHeaderScrolled] = useState<boolean>(false);

  // Effect to handle header dynamics based on scroll position
  useEffect(() => {
    const handleScroll = () => {
      // Set isHeaderScrolled to true if scroll position is beyond 50px from top
  ...
  const handleAnalyze = async () => {
    // Basic validation: Check if feedback text is empty
      ...
      <main className="w-full max-w-4xl bg-white shadow-xl rounded-lg p-8 space-y-8 mt-40 z-0">
        ...
        <section>
          ...
          <div className="bg-gray-50 p-6 rounded-md border border-gray-200">
            ...
            {/* Placeholder message when no analysis result is present */}
            ...
            {/* Display analysis results if available and not loading/error */}
                  ...
                  <span
                    className={`inline-flex items-center px-3 py-0.5 rounded-full text-sm font-medium ${
                      analysisResult.sentiment
                        ...
                        : "bg-gray-100 text-gray-800" // Default if sentiment is not clearly positive/negative/mixed
                    }`}
                  >
                    {analysisResult.sentiment}
                  </span>
